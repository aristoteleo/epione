...
def _main_logger_process(queue, level, formatter):
	"""Standalone function for multiprocessing logger process"""
	
	# Create a new logger for this process
	logger = logging.getLogger("TOBIAS_MP")
	...
	while True:
		try:
			...
			logger.error("Multiprocessing logger lost connection to queue - probably due to an error raised from a child process.")
			break
...
class TobiasLogger(logging.Logger):
	...
	def start_logger_queue(self):
		...
		self.listener = mp.Process(target=_main_logger_process, args=(self.queue, self.level, self.formatter))
		self.listener.start()
	...
	def main_logger_process(self):
		...
		return _main_logger_process(self.queue, self.level, self.formatter)
