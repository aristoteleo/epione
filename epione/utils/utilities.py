...
#----------------------------------- Multiprocessing ---------------------------------------#
#-------------------------------------------------------------------------------------------#
def check_cores(given_cores, logger=None):
	...
	if given_cores > available_cores:
		if logger:
			...
			console.warn(f"Number of available cores is {available_cores} but --cores is set to {given_cores}. Setting --cores to {available_cores}.")
...
def monitor_progress(task_list, logger, prefix="Progress", show_bar=True):

	prev_done = 0
	no_tasks = len(task_list)
	done = sum([task.ready() for task in task_list])
	
	# Check if logger uses epione console
	use_epione_console = hasattr(logger, 'use_epione_console') and logger.use_epione_console
	
	# Initialize progress bar
	if show_bar:
		bar_length = 50
		filled_length = int(bar_length * done // no_tasks)
		bar = '█' * filled_length + '░' * (bar_length - filled_length)
		
		if use_epione_console:
			import epione.utils.console as console
			console.level2(f"🔄 {prefix}: 0% |{bar}| {done}/{no_tasks}")
		else:
			logger.info("{0}: 0% |{1}| {2}/{3}".format(prefix, bar, done, no_tasks))
	else:
		if use_epione_console:
			import epione.utils.console as console
			console.level2(f"🔄 {prefix}: 0%")
		else:
	...
	while done != no_tasks:
		...
		if done != prev_done:
			percent = round(done/no_tasks*100.0, max(0,len(str(no_tasks))-1))
			...
			if show_bar:
				...
				if use_epione_console:
					import epione.utils.console as console
					console.level2(f"🔄 {prefix}: {percent}% |{bar}| {done}/{no_tasks}")
				else:
					logger.info("{0}: {1}% |{2}| {3}/{4}".format(prefix, percent, bar, done, no_tasks))
			else:
				if use_epione_console:
					import epione.utils.console as console
					console.level2(f"🔄 {prefix}: {percent}%")
				else:
					logger.info("{1} {0}%".format(percent, prefix))
			prev_done = done
		else:
			time.sleep(0.1)
	...
	if show_bar:
		...
		if use_epione_console:
			import epione.utils.console as console
			console.level2(f"🔄 {prefix}: 100% |{bar}| {no_tasks}/{no_tasks}")
		else:
			logger.info("{0}: 100% |{1}| {2}/{3}".format(prefix, bar, no_tasks, no_tasks))
	
	if use_epione_console:
		import epione.utils.console as console
		console.success(f"✅ {prefix} completed!")
	else:
...
class Progress:
	...
	def __init__(self, total_elements, logger, prefix="Progress", round=0, show_bar=True):
		...
		self.bar_length = 50  # Length of progress bar
	...
	def write(self, progress):
		...
		if percent_to_write != self.last_written:
			if self.show_bar:
				# Create progress bar
				filled_length = int(self.bar_length * progress // self.total_elements)
				bar = '█' * filled_length + '░' * (self.bar_length - filled_length)
				
				# Check if logger uses epione console
				if hasattr(self.logger, 'use_epione_console') and self.logger.use_epione_console:
					import epione.utils.console as console
					console.level2(f"🔄 {self.prefix}: {percent_to_write}% |{bar}| {progress}/{self.total_elements}")
				else:
				...
				if hasattr(self.logger, 'use_epione_console') and self.logger.use_epione_console:
					import epione.utils.console as console
					console.level2(f"🔄 {self.prefix}: {percent_to_write}%")
				else:
