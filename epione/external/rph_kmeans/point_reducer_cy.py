...
from .point_reducer_base import RPPointReducerBase
from ._point_reducer_cy import get_ary_labels
from ._point_reducer_cy import update_densex_and_weight, update_sparsex_and_weight, update_labels
from ._point_reducer_cy import densex_radius_bkt_improve, sparsex_radius_bkt_improve
from .utils import cal_dist2_ary_sparse, cal_dist2_ary_dense
...
class RPPointReducerCy(RPPointReducerBase):
	def __init__(self, w=None, max_point=2000, proj_num=5, max_iter=1000, sample_dist_num=1000,
		...
		super(RPPointReducerCy, self).__init__(w, max_point, proj_num, max_iter, sample_dist_num,
				bkt_improve, radius_divide, bkt_size_keepr, center_dist_keepr, verbose=verbose, min_point=min_point,
	...
	def fit_transform(self, X):
		...
		pbar = None
		if self.verbose > 1:
			pbar = tqdm(total=self.max_iter, leave=True,
					desc=f"RPH reduce (max_point={self.max_point}, proj={self.proj_num})")
		while iter_num < self.max_iter:
			if group_num <= self.max_point:
				if pbar is not None:
					pbar.set_postfix(group_num=int(group_num))
				break
			...
			# Check min_point before applying label/x updates
			if self.min_point is not None and bkt_num < self.min_point:
				if self.verbose > 0:
					print('Next reduction would produce {} < min_point {}. Stop before applying.'
						.format(bkt_num, self.min_point))
				break
			...
			if pbar is not None:
				if self.verbose > 1:
					...
					pbar.set_postfix(iter=iter_num, group_num=int(group_num), group_buckets=int(group_bkt_count), orphans=int(orphan_point_count))
				else:
					pbar.set_postfix(iter=iter_num, group_num=int(group_num))
				pbar.update(1)
