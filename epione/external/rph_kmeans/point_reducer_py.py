...
from .utils import cal_weighted_ave_sparse_vec, cal_weighted_ave_dense_vec
...
class RPPointReducerPy(RPPointReducerBase):
	...
	def fit_transform(self, X):
		...
		while iter_num < self.max_iter:
			...
			# Prospective new group number after this iteration
			new_group_num = len(group_buckets) + len(orphan_points)
			if self.min_point is not None and new_group_num < self.min_point:
					# Respect min_point without extra logging to keep output clean
					break
			...
			if iter_num == 1:
				...
				if pbar is not None:
					if self.verbose > 1:
						pbar.set_postfix(iter=iter_num, group_num=int(group_num), buckets=len(group_buckets), orphans=len(orphan_points))
					else:
